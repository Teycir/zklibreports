...
/// @notice Echidna property harness for F-MAN-04.
contract EchidnaF4ChallengeBindingHarness {
    ...
    function action_settle() public {
        if (!opened) return;
        try ChallengeBindingStub(challengeAddr).settle(attackerOperator, victimOperator) {} catch {}
    }

    function echidna_victim_not_forced_into_unrelated_challenge() public view returns (bool) {
        (,,, , address victimCurrentChallenge) = rollup.stakers(victimStaker);
        return victimCurrentChallenge == address(0);
    }

    function echidna_victim_not_slashed_by_unrelated_assertions() public view returns (bool) {
