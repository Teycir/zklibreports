...
/// @notice Stateful Medusa harness for TrailblazersBadgesS2.getBadge boundary logic.
contract MedusaTaikoF1GetBadgeBoundaryHarness is TrailblazersBadgesS2Like {
    function action_mint(uint8 _badgeType, uint8 _movementType) public {
        if (totalSupply() >= 8) return;
        mintMock(_badgeType, _movementType);
    }

    function property_unminted_token_must_revert() public returns (bool) {
        if (totalSupply() == 0) {
            return true;
        }
    ...
    function property_first_minted_token_must_be_readable() public returns (bool) {
        if (totalSupply() == 0) {
            return true;
        }
